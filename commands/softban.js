...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('softban')
		.setDescription('Выдает бан молотом бана(может использовать ТОЛЬКО СОЗДАТЕЛЬ СЕРВЕРА)')
	...
	async execute(client, interaction, db) {
		try {
			...
			if (interaction.guild.ownerId !== author.id) return await interaction.reply('Вы не можете использовать эту команду!');
			...
			const fullReason = `${interaction.user.tag} (${author.id}) softban -> ${reason}`;

			const embed = new EmbedBuilder()
				.setTitle(`Молот бана ударил \`${member.displayName}\`!`)
				.setDescription(`${author.toString()} (${author.id}) очень сильно ударил молотом бана участника \`${member.displayName}\` по причине: ${reason}`)
				.setColor(0x12ff00)
...
module.exports.help = {
	name: "**`/softban`**",
	description: 'Банит участников и модераторов\nДоступно только владельцам сервера',
	chapter: 'Moderation',
};